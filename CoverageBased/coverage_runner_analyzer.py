    ...
    if selected_path:
        if os.path.isdir(selected_path):
            for file in os.listdir(selected_path):
                if file.startswith("sorted_test_") and file.endswith(".txt"):
                    full_path = os.path.join(selected_path, file)
                    strat_name = file.replace("sorted_test_", "").replace(".txt", "")
                    args.append((full_path, strat_name, BASE_RUN_DIR))
        else:
            strat_name = os.path.basename(selected_path).replace("sorted_test_", "").replace(".txt", "")
            args = [(selected_path, strat_name, BASE_RUN_DIR)]
    else:
        strategies = [
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        main(sys.argv[1])  # Klasör veya dosya olabilir
    else:
        main()
