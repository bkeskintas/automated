...
LOG_FILE = os.path.join(RESULT_DIR, "process.txt", exist_ok=True)
...
def agent2_generate_multiple_orders(system_prompts, all_tests, regression_tests, allowed_tests):
    ...
    for idx, original_prompt in enumerate(tqdm(system_prompts, desc="Generating test orderings")):
        ...
        if attempt == max_attempts:
            print(f"❌ Strategy #{idx+1} failed after {max_attempts} attempts. Skipping.")
            with open(LOG_FILE, "a", encoding="utf-8") as logf:
                logf.write(f"\n❌ Strategy #{idx+1} failed after {max_attempts} attempts. Skipped.\n")
    return results
...
def main():
    ...
    combined_path = os.path.join(RESULT_DIR, "sorted_test_combined.txt")
