...
def call_openai(prompt, config):
    ...
    token_count = estimate_tokens(prompt, config["model"])
...
def generate_prompts(all_tests, regression_tests, base_risk_order):
    ...
    input_prompt = shared_intro + (
        ...
        "Example: input 'A > C' must become 'A > B > C > D' if B relates to A and D to C."
    )
    relation_prompt = shared_intro + (
        ...
        "Example: input 'A > C' must become 'A > C > B > D' if B relates to A and D to C. A and C have to be first. Also  B need to be before D since A > C and B is related with A."
    )
...
def main():
    ...
    valid_modules = {
        line.split("#")[0].replace("ServiceTest", "").replace("Test", "").strip()
